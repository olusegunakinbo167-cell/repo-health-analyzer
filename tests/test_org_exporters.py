...
from src.models import CategoryScore, CiCdSetup, CommunityFiles, HealthScore, MaintenanceActivity, OrgHealthSummary, OrgRepoScore, RepoMetrics

def make_metrics(full_name: str, stars: int=10, language: str | None='Python') -> RepoMetrics:
    return RepoMetrics(full_name=full_name, description=f'Test {full_name}', stars=stars, language=language, default_branch='main', community_files=CommunityFiles(True, True, False, True), ci_cd=CiCdSetup(['ci.yml'], 1), maintenance=MaintenanceActivity(10, 2, 8, 0))

def make_score(total: float) -> HealthScore:
    return HealthScore(total_score=total, documentation=CategoryScore(name='Documentation', score=20.0, max_score=20.0), maintenance=CategoryScore(name='Maintenance', score=18.0, max_score=25.0), ci_cd=CategoryScore(name='CI/CD', score=20.0, max_score=25.0), governance=CategoryScore(name='Governance', score=17.0, max_score=20.0), academic_impact=CategoryScore('Academic Impact', 0.0, max_score=10.0))

def make_summary() -> OrgHealthSummary:
    return OrgHealthSummary(org='testorg', total_repos=5, analyzed_repos=3, failed_repos=2, avg_score=80.0, median_score=82.0, score_distribution={'A': 1, 'B': 1, 'C': 0, 'D': 1, 'F': 0}, top_repos=[OrgRepoScore('testorg/repo-a', 92.5, 'A', 100, 'Python'), OrgRepoScore('testorg/repo-b', 82.0, 'B', 50, 'Go')], bottom_repos=[OrgRepoScore('testorg/repo-c', 65.0, 'D', 5, 'Rust')], category_averages={'documentation': 20.0, 'maintenance': 18.5, 'ci_cd': 19.0, 'governance': 17.5}, missing_files_stats={'readme': 0, 'license': 1, 'contributing': 2, 'code_of_conduct': 0}, ci_adoption_rate=66.67, total_stars=155, timestamp='2026-01-15T12:00:00Z')

def test_org_json_exporter_basic() -> None:
    """Org JSON exporter includes summary fields and per-repo data."""
    summary = make_summary()
    results = [(make_metrics('testorg/repo-a', 100, 'Python'), make_score(92.5)), (make_metrics('testorg/repo-b', 50, 'Go'), make_score(82.0)), (make_metrics('testorg/repo-c', 5, 'Rust'), make_score(65.0))]
...
def test_org_markdown_exporter_no_results() -> None:
    ...
    summary = OrgHealthSummary(org='empty', total_repos=0, analyzed_repos=0, failed_repos=0, avg_score=0.0, median_score=0.0, score_distribution={'A': 0, 'B': 0, 'C': 0, 'D': 0, 'F': 0}, top_repos=[], bottom_repos=[], category_averages={'documentation': 0.0, 'maintenance': 0.0, 'ci_cd': 0.0, 'governance': 0.0}, missing_files_stats={'readme': 0, 'license': 0, 'contributing': 0, 'code_of_conduct': 0}, ci_adoption_rate=0.0, total_stars=0, timestamp='2026-01-15T12:00:00Z')
    exporter = OrgMarkdownExporter()
    md = exporter.export(summary, results=[])
    assert 'empty' in md
...
def test_render_org_index_table() -> None:
    ...
    results = [(make_metrics('acme/foo', 20, 'Python'), make_score(90.0)), (make_metrics('acme/bar', 5, 'Go'), make_score(60.0))]

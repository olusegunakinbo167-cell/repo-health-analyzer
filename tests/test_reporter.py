...
def make_sample():
    metrics = RepoMetrics(
        ...
        community_files=CommunityFiles(
            readme=True, license=True, contributing=False, code_of_conduct=False
        ),
        ci_cd=CiCdSetup(workflow_files=["ci.yml", "lint.yml"], workflow_count=2),
        maintenance=MaintenanceActivity(
            commits_last_90_days=15, open_issues=3, closed_issues=12, stale_prs=1
        ),
    )
    health = HealthScore(
        total_score=72.0,
        documentation=CategoryScore(
            name="Documentation",
            score=15.0,
            penalties=["Missing CONTRIBUTING.md", "Missing CODE_OF_CONDUCT.md"],
            recommendations=[
        ...
        governance=CategoryScore(
            name="Governance",
            score=18.0,
            penalties=["1 stale PR(s) open >30 days"],
            recommendations=["Review and close/merge stale pull requests"],
        ),
    )
...
def test_render_markdown_perfect_score() -> None:
    ...
    md = render_markdown(metrics, health)
    assert "100" in md
    # Should still have collapsible sections even with no issues
    assert "<details>" in md
...
def test_render_markdown_zero_score() -> None:
    ...
    metrics = RepoMetrics(
        ...
        language=None,
        default_branch="main",
        community_files=CommunityFiles(False, False, False, False),
        ci_cd=CiCdSetup([], 0),
        maintenance=MaintenanceActivity(0, 0, 0, 0),
    )
    health = HealthScore(
        total_score=20.0,
        documentation=CategoryScore(
            "Documentation", 0.0, penalties=["Missing README file"], recommendations=["Add README"]
        ),
        maintenance=CategoryScore(

...
def make_score(total: float) -> HealthScore:
    ...
    return HealthScore(
        ...
        maintenance=cat("Maintenance", 20.0),
        ci_cd=cat("CI/CD", 20.0),
        governance=cat("Governance", 20.0),
    )
...
def make_fake_collector_factory(
    active: List[int], max_seen: List[int], fail_on: set[str] | None = None
):
    def factory() -> RepoCollector:  # type: ignore[return-value]
        return FakeCollector(active, max_seen, fail_on=fail_on)  # type: ignore[return-value]
    return factory
...
async def test_batch_concurrency_throttling() -> None:
    ...
    max_seen = [0]
    config = RepoConfig()
    runner = OrgBatchRunner(
        config,
        options=BatchOptions(concurrency=3),
        collector_factory=make_fake_collector_factory(active, max_seen),
    )

    # Patch score_repo to return deterministic scores
    ...
    batch_mod.score_repo = fake_score_repo  # type: ignore[assignment]
    try:
        repos = [f"org/repo-{i}" for i in range(10)]
        result = await runner.analyze_repos(repos, org="org")
    finally:
        batch_mod.score_repo = orig_score_repo  # type: ignore[assignment]

    assert result.org == "org"
    assert len(result.repos) == 10
    assert len(result.failed) == 0
    # Concurrency should never exceed the configured limit
    assert max_seen[0] <= 3
    assert max_seen[0] >= 2  # actually ran concurrently
...
async def test_batch_error_isolation() -> None:
    ...
    runner = OrgBatchRunner(
        ...
        collector_factory=make_fake_collector_factory(
            active, max_seen, fail_on=fail_on
        ),
    )
...
async def test_batch_progress_callback() -> None:
    ...
    orig_score_repo = batch_mod.score_repo
    batch_mod.score_repo = lambda m, c: make_score(80.0)  # type: ignore[assignment]
    ...
    try:
        ...
        result = await runner.analyze_repos(
            repos, org="org", progress_callback=on_progress
        )
    finally:
        batch_mod.score_repo = orig_score_repo  # type: ignore[assignment]

...
def make_metrics(
    ...
    *,
    ...
    return RepoMetrics(
        ...
        ci_cd=CiCdSetup(
            workflow_files=["ci.yml"] if has_ci else [],
            workflow_count=1 if has_ci else 0,
        ),
...
def make_score(
    ...
    def cat(name: str, score: float) -> CategoryScore:
        return CategoryScore(name=name, score=score)

    return HealthScore(
        total_score=total,
        documentation=cat("Documentation", doc),
        maintenance=cat("Maintenance", maint),
        ci_cd=cat("CI/CD", ci),
        governance=cat("Governance", gov),
    )
...
def test_aggregate_org_health_basic() -> None:
    ...
    results = [
        (make_metrics("org/repo-a", stars=100), make_score(95.0, 24, 24, 24, 23)),  # A
        (make_metrics("org/repo-b", stars=50), make_score(82.0, 20, 22, 20, 20)),  # B
        (make_metrics("org/repo-c", stars=10), make_score(65.0, 15, 18, 17, 15)),  # D
    ]
    ...
    # Score stats
    assert summary.avg_score == 80.67  # (95 + 82 + 65) / 3
    ...
    # Category averages
    assert summary.category_averages["documentation"] == 19.67  # (24+20+15)/3
    assert summary.category_averages["maintenance"] == 21.33
    assert summary.category_averages["ci_cd"] == 20.33
    assert summary.category_averages["governance"] == 19.33
...
def test_aggregate_org_health_missing_files_and_ci() -> None:
    ...
    # Missing file counts (repos MISSING each file)
    ...
    # CI adoption: 2 out of 3 repos have CI
...
def test_aggregate_org_health_top_n_limit() -> None:
    ...
    results = [
        (make_metrics(f"org/repo-{i:02d}", stars=i), make_score(float(100 - i)))
        for i in range(20)
    ]
    ...
    assert summary.bottom_repos[0].score == 81.0  # lowest of the 20

    # Full ranking with top_n larger than repo count

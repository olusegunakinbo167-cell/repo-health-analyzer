...
import pytest
from src.exporters import JSONExporter, MarkdownExporter, ReportMetadata, export_report, get_exporter_for_path
...
def test_json_exporter_with_plugin_statuses():
    metrics, health = _make_test_objects()
    exporter = JSONExporter()
    plugin_statuses = [PluginStatus(name='fandango', available=True, version=None, cli_path='/tmp/fandango.js', error=None), PluginStatus(name='embark', available=False, version=None, cli_path=None, error='CLI not found')]
...
def test_json_exporter_with_metadata():
    metrics, health = _make_test_objects()
    exporter = JSONExporter()
    metadata = ReportMetadata(repository='octocat/Hello-World', commit_sha='abc123def456789', timestamp='2026-07-25T10:00:00Z', tool_version='0.2.1')
...
def test_markdown_exporter_with_plugin_statuses():
    metrics, health = _make_test_objects()
    exporter = MarkdownExporter()
    plugin_statuses = [PluginStatus(name='fandango', available=True, cli_path='/home/user/.openclaw/extensions/fandango/fandango.js', error=None), PluginStatus(name='embark', available=False, cli_path=None, error='embark CLI not found')]
    output = exporter.export(metrics, health, plugin_statuses=plugin_statuses)
...
def test_export_report_format_override(tmp_path):
    ...
    result_path = export_report(metrics, health, output_path, format='json')
...
def test_check_plugin_status_fandango_found(monkeypatch):
    ...
    with patch.object(fandango_mod, 'find_fandango_cli', return_value=Path('/fake/fandango.js')):
...
def test_check_plugin_status_embark_not_found(monkeypatch):
    ...
    with patch.object(embark_mod, 'find_embark_cli', side_effect=FileNotFoundError('embark CLI not found')):
